...
class Solution:
    def dominant_pairs(self, n, arr):
        mid = n // 2
        arr = sorted(arr[:mid]) + sorted(arr[mid:])
        left, right, count = 0, mid, 0
        while left < mid:
            # right pointer moves forward only as backward will be not valid conditions due to sorted nature
            while right < n and arr[left] >= 5 * arr[right]:
                right += 1

            # Addition of all dominant pairs before the current right (including the right) 
            count += right - n // 2
            left += 1
        return count
...
def main():
    ...
    solution = Solution()
    print(solution.dominant_pairs(len(arr), arr))
